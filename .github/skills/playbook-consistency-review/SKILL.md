---
name: playbook-consistency-review
description: Use this skill when the user asks to review, normalize, or enforce consistency across Solution Cookbook playbooks and variants, including frontmatter, architecture section order, terminology, and diagram naming/linking.
---

# Playbook Consistency Review

Use this skill to run a repeatable, high-consistency review and normalization pass across reference playbooks and variants in the `solution-cookbook` repository only.
...
Collect the following before running:
- explicit variant playbook folder list
- mode: edit-in-place or review-only

Canonical reference playbooks (fixed):
...
- Re-run the same process after adding new variants to preserve drift-free metadata and structure.
- For analysis-only mode, use: `Review only; do not modify files.`
- Keep all edits inside `playbooks/**` unless explicitly asked to change repository-level docs.
